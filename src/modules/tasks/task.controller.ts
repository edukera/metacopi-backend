...
export class TaskController {
  ...
  @ApiResponse({ status: 200, description: 'Returns a list of all tasks.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @AuthenticatedUser
  @CheckPermission('Task', 'list')
  async findAll(@Query('classId') classId?: string): Promise<Task[]> {
    if (classId) {
  ...
  @ApiResponse({ status: 200, description: 'Returns the list of user\'s tasks.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @AuthenticatedUser
  @CheckPermission('Task', 'read')
  async findMyTasks(): Promise<Task[]> {
  ...
  @ApiResponse({ status: 200, description: 'Returns the specified task.' })
  ...
  @ApiParam({ name: 'id', description: 'Task ID' })
  @ApiResponse({ status: 200, description: 'The task has been successfully archived.' })
  @ApiResponse({ status: 404, description: 'Task not found.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @AuthenticatedUser
  @CheckPermission('Task', 'archive')
  async archive(@Param('id') id: string): Promise<Task> {
  ...
  @ApiParam({ name: 'id', description: 'Task ID' })
  @ApiResponse({ status: 200, description: 'The task has been successfully published.' })
  @ApiResponse({ status: 404, description: 'Task not found.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @AuthenticatedUser
